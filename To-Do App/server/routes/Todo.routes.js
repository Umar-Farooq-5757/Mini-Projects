...
todoRouter.post("/create", async (req, res) => {
  try {
    ...
    const todo = await db.execute(
      ...
      [crypto.randomUUID(), text, "fcfd64e1-6de5-4936-a612-fd1bf0b69218", false]
    );
...
todoRouter.delete("/delete/:id", async (req, res) => {
  try {
    ...
    const todoToDelete = await db.execute(
      "SELECT * FROM tasks_fullstack WHERE task_id=?",
      [todoId]
    );
    if (!todoToDelete) {
      return res.json({ success: false, message: "Task to delete not found" });
    }
    await db.execute("DELETE FROM tasks_fullstack WHERE task_id=?", [todoId]);
...
todoRouter.put("/edit/:id", async (req, res) => {
  try {
    ...
    if (!text) {
      return res.json({ success: false, message: "Todo doesn't exist" });
    }
    await db.execute(
      "UPDATE tasks_fullstack SET text=?, isCompleted=? WHERE task_id=?",
